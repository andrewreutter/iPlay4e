...
Modalbox.Methods = {
	...
	show: function(content, options) {
        ...
        // ANDREW put this here.
        $('MB_content').update( '<img src="/images/DivLoadingSpinner.gif">' );
			...
			setTimeout(function() { // MSIE sometimes doesn't display content correctly
				Modalbox.resize(0, $(this.MBcontent).getHeight() - $(this.MBwindow).getHeight() + $(this.MBheader).getHeight(), {
					afterResize: function(){
						...
						this._setFocus(); // Setting focus on first 'focusable' element in content (input, select, link or button)
	...
	_setFocus: function() { 
		...
		if(this.focusableElements.length > 0 && this.options.autoFocusing == true) {
			...
			try { firstEl.focus(); } catch (e) {} // Focus on first focusable element except close button
	...
	_findFocusableElements: function(){ // Collect form elements or links from MB content
		this.MBcontent.select('input:not([type~=hidden]), select, button, a[href]').invoke('addClassName', 'MB_focusable');
			...
			case Event.KEY_TAB:
				...
				if(!event.shiftKey) { //Focusing in direct order
					if(this.currFocused == this.focusableElements.length - 1) {
						try { this.focusableElements.first().focus(); } catch (e) {}
						this.currFocused = 0;
					} else {
						this.currFocused++;
						try { this.focusableElements[this.currFocused].focus(); } catch (e) {}
					}
				} else { // Shift key is pressed. Focusing in reverse order
					if(this.currFocused == 0) {
						try { this.focusableElements.last().focus(); } catch (e) {}
	...
	_setPosition: function () {
		...
		$(this.MBwindow).setStyle({left: '10px'});
	},
